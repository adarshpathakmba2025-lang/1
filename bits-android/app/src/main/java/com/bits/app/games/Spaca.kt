...
/**
 * Spaca: a formation of drones sways above you, peels off to dive, and you shoot it down.
 *
...
data class SpacaShot(val x: Float, val y: Float)

data class SpacaDrone(
    val id: Int,
    val col: Int,
    val row: Int,
...
data class SpacaState(
    val shipX: Float,
    val drones: List<SpacaDrone>,
    val shots: List<SpacaShot>,
    val bombs: List<SpacaShot>,
    val swayX: Float,
    val swayDir: Int,
    val wave: Int,
    val lives: Int,
...
object Spaca {
    ...
    private fun formation(wave: Int, startId: Int): List<SpacaDrone> {
        var id = startId
        return buildList {
    ...
    fun positionOf(state: SpacaState, drone: SpacaDrone): Pair<Float, Float> =
        if (drone.diving) {
            drone.x to drone.y
        } else {
    ...
    fun steer(state: SpacaState, x: Float): SpacaState =
        if (state.dead) state
        else state.copy(shipX = x.coerceIn(SHIP_W / 2f, 1f - SHIP_W / 2f), started = true)

    fun nudge(state: SpacaState, dx: Float): SpacaState = steer(state, state.shipX + dx)

    fun start(state: SpacaState): SpacaState = if (state.dead) state else state.copy(started = true)

    fun step(state: SpacaState, random: Random = Random.Default): SpacaState {
        ...
        val newBombs = buildList {
            launched.forEach { drone ->
                ...
                if (random.nextFloat() < chance) {
                    ...
                    add(SpacaShot(dx, dy + DRONE_H / 2f))
                }
            }
        }
        ...
        val firedShots = if (reload <= 0) {
            reload = RELOAD_TICKS
            swayed.shots + SpacaShot(swayed.shipX, SHIP_Y - SHIP_H / 2f)
        } else {
            swayed.shots
        }
